...
<form class="grid px-8 gap-2.5 items-center justify-center w-full">
  <div class="flex flex-col gap-2.5">
    {#if !loaded}
      ...
      <div>
        <button 
          class="link text-primary label-text-alt" 
          type="button" 
          on:click={() => (show = !show)}
        >
          {show ? 'Hide custom config upload' : 'Use custom config'}
        </button>
        {#if show}
          <label class="grid mt-2">
            <span>Upload the custom config file:</span>
    ...
    {#if loaded}
      ...
      {#if !show && !temp}
        <label class="grid label-text-alt gap-2.5 mt-4">
        ...
        <label class="grid label-text-alt gap-2.5 mt-2">
      ...
      {#if show || temp}
        <p class="mt-4">Generated using custom config.</p>
      {/if}
